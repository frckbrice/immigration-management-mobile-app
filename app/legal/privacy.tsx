...
export default function PrivacyPolicyScreen() {
  ...
  const loading = useLegalStore((state) => state.loading.privacy);
  const error = useLegalStore((state) => state.error.privacy);
  const getDocument = useLegalStore((state) => state.getDocument);
  const cacheEntry = useMemo(
    () => getDocument("privacy", currentLanguage),
  ...
  useEffect(() => {
    fetchDocument("privacy", currentLanguage).catch(() => {
      // handled via store error state
    });
  }, [currentLanguage, fetchDocument]);

  const handleRefresh = useCallback(() => {
    fetchDocument("privacy", currentLanguage, { force: true }).catch(() => {
      // handled via store error state
    });
  }, [currentLanguage, fetchDocument]);
  ...
  const errorBackground = withOpacity(
    colors.danger ?? "#EF4444",
    appTheme.dark ? 0.24 : 0.12,
  );
  const errorBorder = withOpacity(
    colors.danger ?? "#EF4444",
    appTheme.dark ? 0.45 : 0.28,
  );
            ...
            <Text style={[styles.metaText, { color: colors.muted }]}>
              {lastSyncedLabel
                ? t("legal.lastUpdated", {
                    ...
                    date: lastSyncedLabel,
                  })
                : t("legal.lastUpdatedUnknown", {
                    defaultValue: "Last updated recently",
                  })}
            </Text>
          ...
          {isInitialLoading ? (
            ...
            >
              ...
              <View style={styles.feedbackTextGroup}>
                ...
                <Text style={[styles.feedbackMessage, { color: colors.muted }]}>
                  {error}
                </Text>
              </View>
              <Pressable
              ...
              style={[
                styles.contentCard,
                {
                  ...
                  borderColor: withOpacity(
                    colors.text,
                    appTheme.dark ? 0.12 : 0.08,
                  ),
                  shadowColor: withOpacity(
                    colors.text,
                    appTheme.dark ? 0.45 : 0.2,
                  ),
                },
              ]}
            >
              {paragraphs.map((paragraph, index) => (
                <Text
                  key={index}
                  style={[styles.contentParagraph, { color: colors.text }]}
                >
                  {paragraph}
                </Text>
              ))}
              {!paragraphs.length && (
                <Text
                  style={[styles.contentParagraph, { color: colors.muted }]}
                >
                  —
                </Text>
              )}
          ...
          {hasContent && error && (
            ...
            >
              ...
              <Text style={[styles.noticeText, { color: colors.muted }]}>
                {t("legal.refreshFailed", {
                  defaultValue:
                    "Unable to refresh right now. Showing the last saved version.",
                })}
              </Text>
            </View>
          )}
