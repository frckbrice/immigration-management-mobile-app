...
export default function CaseDetailsScreen() {
  ...
  const statusTimeline = useMemo(() => {
    const status =
      normalizeStatus((selectedCase as any)?.status) || "submitted";
  ...
  const handleMessageAdvisor = () => {
    ...
    if (statusKey !== "under_review") {
      showAlert({
        title: t("cases.chatUnavailableTitle", {
          defaultValue: "Chat Not Available",
        }),
        message: t("cases.chatUnavailableMessage", {
          defaultValue:
            "Chat will be available once your advisor reviews this case.",
        }),
    ...
    if (!(caseData as any)?.assignedAgent) {
      showAlert({
        title: t("cases.chatAwaitingAgentTitle", {
          defaultValue: "Advisor Pending",
        }),
        message: t("cases.chatAwaitingAgentMessage", {
          defaultValue:
            "An advisor will contact you shortly. Chat becomes available after the assignment.",
        }),
        actions: [{ text: t("common.close"), variant: "primary" }],
      });
      return;
          ...
          >
            ...
            >
              ...
              <Text style={[styles.caseTitle, { color: theme.colors.text }]}>
                {formatServiceTypeLabel(caseData?.serviceType) ||
                  t("cases.title")}
              </Text>
              ...
              {caseData?.assignedAgent ? (
                <View style={styles.agentRow}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    >
                      {t("caseDetails.immigrationAdvisor")}
                    </Text>
                  </View>
                  <Pressable
                    style={styles.primaryBtn}
                    onPress={handleMessageAdvisor}
                  >
                    <Text style={styles.primaryBtnText}>
                      {t("caseDetails.messageAdvisor")}
                    </Text>
                  </Pressable>
                </View>
              ) : (
                <View style={styles.emptyBox}>
                  ...
                  >
                    {t("caseDetails.noAdvisorAssigned")}
                  </Text>
                </View>
              )}
            </View>
                ...
                documents.map((doc) => (
                  <View key={doc.id} style={styles.docRow}>
                    <View style={styles.docIcon}>
                      <IconSymbol name="doc.fill" size={18} color="#2196F3" />
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text
                      ...
                      >
                        ...
                        {formatDateLabel(doc.uploadDate)}
                      </Text>
                    </View>
                  </View>
                ))
