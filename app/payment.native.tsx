...
export default function PaymentScreen() {
  ...
  const defaultDescription = t("payments.basicTierDescription", {
    defaultValue: "Basic Tier - One-time payment",
  });
  ...
  const tier =
    (tierParam as "basic" | "standard" | "premium") ||
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (!paymentIntent.clientSecret) {
        logger.error("Payment intent missing client secret", {
          paymentIntentId: paymentIntent.id,
        });
        throw new Error(
          "Backend did not return client secret for payment confirmation",
        );
      }

      // Step 2: Confirm the payment with Stripe SDK
      logger.info("Confirming payment intent with Stripe", {
        paymentIntentId: paymentIntent.id,
      });
      ...
      if (confirmedIntent) {
        ...
        try {
          ...
          const verifiedPayment = await paymentsService.verifyPayment(
            stripePaymentIntentId,
          );

          if (
            verifiedPayment.paymentStatus === "COMPLETED" ||
            verifiedPayment.stripeStatus === "succeeded"
          ) {
            ...
            try {
              ...
              logger.warn(
                "Failed to refresh subscription status after payment",
                refreshError,
              );
            ...
            showToast({
              ...
              message: t("payments.paymentSuccessfulMessage", {
          ...
          logger.error(
            "Payment verification failed after Stripe confirmation",
            {
              paymentIntentId: stripePaymentIntentId,
