...
export default function PaymentHistoryScreen() {
  ...
  const loadOutstandingCases = useCallback(async () => {
    ...
    try {
      ...
      const outstanding = result.filter(
        (item) => !["REJECTED", "CLOSED"].includes(item.status),
      );
  ...
  const openCaseSheet = useCallback(async () => {
    console.log("[Payments] openCaseSheet tapped", {
      existing: caseOptions.length,
    });
    if (!caseOptions.length) {
      try {
        ...
        console.warn(
          "[Payments] Failed to load cases before presenting sheet",
          error,
        );
      }
    }
        ...
        >
          ...
          >
            <View style={styles.summaryRow}>
              <View style={styles.summaryItem}>
                ...
                >
                  {t("payments.history.summary.totalPaid")}
                </Text>
              ...
              <View style={styles.summaryItem}>
                ...
                >
                  {t("payments.history.summary.transactions")}
                </Text>
          ...
          {isLoading ? (
            ...
            payments.map((payment) => {
              const caseReference =
                payment.caseNumber || payment.metadata?.caseNumber || null;
              return (
                <Pressable
                  key={payment.id}
                  style={[
                ...
                >
                    ...
                    <View style={styles.paymentInfo}>
                      ...
                      >
                        {t("payments.history.caseLabel", {
                          caseNumber:
                            caseReference ?? t("payments.history.caseUnknown"),
                        })}
                      </Text>
                    </View>
                  ...
                  <View
                    style={[
                      styles.divider,
                      { backgroundColor: theme.dark ? "#1F2937" : "#E0E0E0" },
                    ]}
                  />
          ...
          {!isLoading && payments.length === 0 && (
            <View style={styles.emptyState}>
              <IconSymbol
                name="creditcard"
                size={64}
                color={theme.dark ? "#FFB020" : "#F97316"}
              />
              <Text
          ...
          <Pressable style={styles.newPaymentButton} onPress={openCaseSheet}>
            <IconSymbol name="plus.circle.fill" size={24} color="#fff" />
            <Text style={styles.newPaymentButtonText}>
              {t("payments.history.actions.makePayment")}
            </Text>
          </Pressable>
        </ScrollView>
      </SafeAreaView>
        ...
        >
          <View style={styles.sheetHeader}>
            <Text style={[styles.sheetTitle, { color: theme.colors.text }]}>
              {t("payments.history.sheet.title")}
            </Text>
            <Text
              style={[
          ...
          {casesLoading ? (
              ...
              {caseOptions.map((caseItem) => (
                ...
                >
                  <View style={styles.caseHeader}>
                    ...
                    <View style={styles.amountPill}>
                      <Text style={styles.amountPillText}>
                        ${deriveCaseAmount(caseItem).toFixed(2)}
                      </Text>
                    </View>
                  </View>
                  <Text
                  ...
                  <View style={styles.caseMetaRow}>
                    <View style={styles.caseMetaItem}>
                      <IconSymbol
                        name="chart.bar.fill"
                        size={14}
                        color={theme.dark ? "#F97316" : "#F97316"}
                      />
                      <Text
                        style={[
                          styles.caseMetaText,
                    ...
                    <View style={styles.caseMetaItem}>
                      ...
                      >
                        {t("payments.history.priorityLabel", {
                          priority: getPriorityLabel(caseItem.priority),
                        })}
                      </Text>
                    </View>
                  </View>
                  <View style={styles.caseMetaRowSecondary}>
                    ...
                    <Text
                      style={[
                        styles.caseMetaTextSecondary,
                        { color: theme.dark ? "#94A3B8" : "#64748B" },
                      ]}
                    >
                      {t("payments.history.sheet.secondaryInfo", {
                        defaultValue:
                          "Tap to review and pay this case in full.",
                      })}
                    </Text>
                  </View>
                </Pressable>
            ...
            <View style={styles.sheetEmptyState}>
              <IconSymbol
                name="checkmark.seal"
                size={48}
                color={theme.dark ? "#1D4ED8" : "#94A3B8"}
              />
              <Text
                style={[styles.sheetEmptyTitle, { color: theme.colors.text }]}
              >
                {t("payments.history.sheet.empty.title")}
              </Text>
              <Text
                style={[
                  styles.sheetEmptySubtitle,
                  { color: theme.dark ? "#94A3B8" : "#64748B" },
                ]}
              >
                {" "}
                {t("payments.history.sheet.empty.subtitle")}{" "}
              </Text>
            </View>
          )}
