...
export default function FAQScreen() {
  ...
  const softSurface = appTheme.dark
    ? withOpacity(colors.surfaceAlt ?? colors.surface, 0.6)
    : withOpacity(colors.text, 0.04);
  const borderColor = appTheme.dark
    ? withOpacity(colors.onSurface ?? colors.text, 0.12)
  ...
  const { grouped, filteredList } = useMemo(() => {
    ...
    const groupedMap = filtered.reduce(
      (acc: Record<string, FAQItem[]>, item) => {
  ...
  const renderCategory = ({ item: category }: { item: string }) => {
    ...
    const pillBackground = withOpacity(
      colors.primary,
      appTheme.dark ? 0.35 : 0.12,
    );
    const pillBorder = withOpacity(colors.primary, appTheme.dark ? 0.6 : 0.3);
    const showMutedState = items.every((faq) => faq.isActive === false);
    const cardBorderColor = showMutedState
      ? withOpacity(colors.text, appTheme.dark ? 0.08 : 0.12)
      : borderColor;
        ...
        {items.map((faq) => {
          ...
          return (
            ...
            >
              ...
              {isExpanded && (
                <View
                  style={[styles.answerBlock, { backgroundColor: softSurface }]}
                >
          ...
          ListHeaderComponent={
            <View style={styles.listHeader}>
              <View
                style={[
                  styles.heroCard,
                  {
                    backgroundColor: withOpacity(
                      colors.primary,
                      appTheme.dark ? 0.22 : 0.1,
                    ),
                    borderColor: withOpacity(
                      colors.primary,
                      appTheme.dark ? 0.45 : 0.25,
                    ),
              ...
              >
                ...
                <View style={styles.heroCopy}>
                  <Text style={[styles.heroTitle, { color: colors.text }]}>
                    {t("profile.faqHeroTitle", {
                      defaultValue: "How can we help today?",
                    })}
                  </Text>
                  <Text style={[styles.heroSubtitle, { color: colors.muted }]}>
                    {t("profile.faqHeroSubtitle", {
                      defaultValue:
                        "Browse our curated knowledge base or reach out to support if you need a hand.",
                    })}
                  </Text>
                </View>
                <Pressable
                ...
                >
                  ...
                  />
                  <Text
                    style={[styles.heroButtonText, { color: colors.onPrimary }]}
                  >
              ...
              <FormInput
                ...
                placeholder={t("profile.faqSearchPlaceholder", {
                  defaultValue: "Search questions or keywords",
                })}
            ...
            !(isLoading && !refreshing) ? (
              ...
              >
                ...
                <Text style={[styles.emptySubtitle, { color: colors.muted }]}>
                  {t("profile.faqEmptyDescription", {
                    defaultValue:
                      "Try a different keyword or reach out to our support specialists.",
                  })}
                </Text>
                <Pressable
                  style={[
