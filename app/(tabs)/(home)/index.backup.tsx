...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await Promise.all([fetchCases(), fetchUnreadCount(), fetchMessages(), fetchDocuments()]);
      } finally {
        if (isActive) {
          refreshStats(true);
        }
  ...
  }, [isAuthenticated, user?.uid, fetchCases, fetchUnreadCount, fetchMessages, fetchDocuments, refreshStats]);
