...
const formatDocumentType = (docType?: string) =>
  docType ? docType.toLowerCase() : "other";
...
const deriveTemplateFileType = (template: Template) => {
  ...
  const urlExtension = template.downloadUrl
    ?.split("?")[0]
    ?.split(".")
    .pop()
    ?.toLowerCase();
  ...
  const fallbackUrlExtension = template.fileUrl
    ?.split("?")[0]
    ?.split(".")
    .pop()
    ?.toLowerCase();
...
const getTemplateIconName = (template: Template) => {
  ...
  if (type.includes("doc") || type.includes("word") || type.includes("rtf"))
    return "file-word-box";
  if (type.includes("xls") || type.includes("csv") || type.includes("sheet"))
    return "file-excel-box";
  if (type.includes("ppt") || type.includes("key"))
...
const getTemplateAccentColor = (
  ...
  if (
    ["jpg", "jpeg", "png", "gif", "bmp", "heic", "svg", "webp"].includes(type)
  )
...
export default function DocumentsScreen() {
  ...
  const [activeTemplateDownloadId, setActiveTemplateDownloadId] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    ...
    const ensureTemplates = async () => {
      try {
        ...
        if (!templates.length) {
          showAlert({
            title: t("documents.templatesErrorTitle", {
              defaultValue: "Unable to load templates",
            }),
            message:
              error?.message ||
              t("documents.templatesErrorMessage", {
  ...
  const triggerWebDownload = useCallback(
    async (uri: string | null | undefined, fileName: string) => {
  ...
  const handleTemplateDownload = useCallback(
    async (template: Template) => {
      ...
      try {
        ...
        setDownloads((prev) => [
          stored,
          ...prev.filter((item) => item.id !== stored.id),
        ]);
        ...
        if (Platform.OS === "web") {
          await triggerWebDownload(
            download.remoteUrl || download.localUri,
            preferredFileName,
          );
        }

        showToast({
          title: t("documents.downloadSuccessTitle", {
            defaultValue: "Template downloaded",
          }),
          message: t("documents.downloadSuccessMessage", {
            defaultValue:
              "You can find this template in the Downloads section.",
          }),
          type: "success",
        });
      } catch (e: any) {
        showAlert({
          title: t("documents.downloadFailedTitle", {
            defaultValue: "Download failed",
          }),
          message:
            e?.message ||
            t("documents.downloadFailedMessage", {
              defaultValue: "Unable to download this template.",
            }),
          actions: [{ text: t("common.close"), variant: "primary" }],
        });
      } finally {
  ...
  const handleDownloadShare = useCallback(
    async (item: DownloadRecord) => {
      try {
        if (!item.localUri) {
          showAlert({
            title: t("documents.shareUnavailableTitle", {
              defaultValue: "Share unavailable",
            }),
            message: t("documents.shareUnavailableMessage", {
              defaultValue:
                "We could not access the downloaded file on this device. Try downloading again.",
            }),
            actions: [{ text: t("common.close"), variant: "primary" }],
          });
          return;
        ...
        if (!canShare) {
          showAlert({
            ...
            message: t("documents.shareUnsupportedMessage", {
              defaultValue: "Sharing is not supported on this device.",
            }),
            actions: [{ text: t("common.close"), variant: "primary" }],
          });
          return;
        ...
        showAlert({
          title: t("documents.shareFailedTitle", {
            defaultValue: "Share failed",
          }),
          message:
            error?.message ||
            t("documents.shareFailedMessage", {
              defaultValue: "Unable to share this file right now.",
            }),
          actions: [{ text: t("common.close"), variant: "primary" }],
        });
      }
  ...
  const handleDownloadDelete = useCallback(
    async (item: DownloadRecord) => {
      try {
        ...
        showToast({
          message: t("documents.downloadRemovedMessage", {
            defaultValue: "Removed from Downloads.",
          }),
          type: "success",
        });
      } catch (error: any) {
        ...
        showAlert({
          title: t("documents.downloadRemoveFailedTitle", {
            defaultValue: "Delete failed",
          }),
          message:
            error?.message ||
            t("documents.downloadRemoveFailedMessage", {
              defaultValue: "Unable to remove this download.",
            }),
          actions: [{ text: t("common.close"), variant: "primary" }],
        });
      }
  ...
  const handleUploadNavigation = useCallback(() => {
    if (!cases || cases.length === 0) {
      showAlert({
        title: t("documents.noCaseTitle", {
          defaultValue: "No case available",
        }),
        message: t("documents.noCaseMessage", {
          defaultValue: "Create a case before uploading documents.",
        }),
        actions: [{ text: t("common.close"), variant: "primary" }],
      });
      return;
  ...
  const activeSearchValue =
    activeTab === "documents" ? searchQuery : templateSearch;
  const onSearchChange =
    activeTab === "documents" ? setSearchQuery : setTemplateSearch;
  ...
  const chipActiveBackground = withOpacity(
    colors.primary,
    theme.dark ? 0.55 : 0.2,
  );
  ...
  const showDocumentsLoader =
    isLoading && documents.length === 0 && !isRefreshingDocuments;
    ...
    <>
      ...
      >
        <View style={styles.header}>
          ...
          >
            <IconSymbol
              name={showFilters ? "xmark" : "slider.horizontal.3"}
              size={20}
              color={colors.text}
            />
          </Pressable>
        ...
        <View style={[styles.tabBar, { backgroundColor: surfaceCard }]}>
          {(["documents", "templates", "downloads"] as ActiveTab[]).map(
            (tab) => (
              <Pressable
                key={tab}
                style={[
              ...
              >
                ...
                >
                  ...
                  {tab === "templates" &&
                    t("documents.tabs.templates", {
                      defaultValue: "Templates",
                    })}
                  {tab === "downloads" &&
                    t("documents.tabs.downloads", {
        ...
        {activeTab === "documents" && showFilters && (
          <View
            style={[styles.filtersContainer, { backgroundColor: surfaceCard }]}
          >
            <View style={styles.filterGroup}>
              <Text style={[styles.filterLabel, { color: colors.text }]}>
                {t("documents.filterByCase")}
              </Text>
              <ScrollView
                horizontal
            ...
            <View style={styles.filterGroup}>
              ...
              <View style={styles.filterRow}>
                {(["all", "pdf", "doc", "image"] as DocumentFilter[]).map(
                  (type) => (
                    <Pressable
                      key={type}
                      style={[
            ...
            <View style={styles.filterGroup}>
              ...
              <View style={styles.filterRow}>
                {(
                  ["all", "pending", "approved", "rejected"] as DocumentStatus[]
                ).map((status) => (
                  <Pressable
                    key={status}
                    style={[
        ...
        >
          {activeTab === "documents" && (
            <>
              <Pressable
                style={[styles.uploadCard, { backgroundColor: surfaceCard }]}
              ...
              >
                ...
                <View style={styles.uploadContent}>
                  ...
                  <Text
                    style={[styles.uploadDescription, { color: colors.muted }]}
                  >
              ...
              {documents.map((document) => {
                ...
                const typeColor =
                  documentTypeColors[
                    documentType as keyof typeof documentTypeColors
                  ] ?? documentTypeColors.default;
                      ...
                      try {
                        ...
                        if (!targetUrl) {
                          showAlert({
                            ...
                            message: t("documents.downloadOpenErrorMessage", {
                              defaultValue:
                                "This document cannot be opened on your device.",
                            }),
                            actions: [
                              { text: t("common.close"), variant: "primary" },
                            ],
                          });
                          return;
                        }
                  ...
                  >
                    ...
                    <View style={styles.documentContent}>
                      ...
                      <View style={styles.documentMeta}>
                        <Text
                          style={[styles.documentSize, { color: colors.muted }]}
                        >
                          {documentSize}
                        </Text>
                        <Text
                          style={[styles.documentDot, { color: colors.muted }]}
                        >
                          •
                        </Text>
                        <Text
                          style={[styles.documentDate, { color: colors.muted }]}
                        >
                          {documentDate}
                        </Text>
                      </View>
                        ...
                        onPress={async () => {
                          try {
                            ...
                            showAlert({
                              ...
                              message: t("documents.downloadFailedMessage", {
                                defaultValue:
                              ...
                              { text: t("common.cancel"), style: "cancel" },
                              {
                                ...
                                onPress: async () => {
                                  ...
                                  try {
                                    ...
                                    await fetchDocuments(buildFilters(), {
                                      force: true,
                                    });
                                    showToast({
                    ...
                    <View style={styles.emptyContainer}>
                      ...
                      <Text style={[styles.emptyText, { color: colors.muted }]}>
                        {t("documents.noTemplates", {
                          defaultValue: "No templates available right now.",
                        })}
                      </Text>
                    </View>
                  ) : (
                    filteredTemplates.map((template) => {
                      const isDownloadingTemplate =
                        activeTemplateDownloadId === template.id;
                      const accentColor = getTemplateAccentColor(
                        template,
                        colors,
                      );
                      ...
                      return (
                        <Pressable
                          ...
                          accessibilityHint={t("documents.tapToDownload", {
                        ...
                        >
                              ...
                              <View style={styles.templateMetaRow}>
                                ...
                                <View style={styles.templateMetaItem}>
                                  ...
                                  >
                                    {(
                                      template.fileType ||
                                      deriveTemplateFileType(template) ||
                            ...
                            <View style={styles.templateHeaderAside}>
                              {template.isRequired ? (
                                ...
                                >
                                  <Text
                                    style={[
                                      styles.templateRequiredText,
                                      { color: colors.danger },
                                    ]}
                                  >
                                    {t("documents.requiredTemplate", {
                          ...
                          <View style={styles.templateActionsRow}>
                            ...
                            >
                              ...
                              <Text
                                style={[
                                  styles.templatePrimaryButtonText,
                                  { color: colors.onPrimary },
                                ]}
                              >
                            ...
                            <Pressable
                              ...
                              accessibilityRole="button"
                              accessibilityHint={t(
                                "documents.previewTemplateHint",
                                { defaultValue: "Open template preview" },
                              )}
                            >
                              <MaterialCommunityIcons
                                name="eye-outline"
                              ...
                              <Text
                                style={[
                                  styles.templateSecondaryButtonText,
                                  { color: colors.primary },
                                ]}
                              >
                                {t("documents.previewTemplate", {
                ...
                sortedDownloads.map((item) => {
                  ...
                  const sizeLabel =
                    typeof item.fileSize === "number"
                      ...
                      : undefined;
                  const metaPrimaryParts = [typeLabel, sizeLabel]
                    .filter(Boolean)
                    .join(" • ");
                  ...
                  return (
                    ...
                    >
                      ...
                      <View style={styles.downloadContent}>
                        <Text
                          style={[styles.downloadName, { color: colors.text }]}
                          numberOfLines={1}
                        >
                          {item.name}
                        </Text>
                        ...
                        <Text
                          style={[styles.downloadMeta, { color: colors.muted }]}
                        >
                          {t("documents.downloadedAt", {
                            defaultValue: "Downloaded at {{date}}",
                            date: formatDownloadDate(item.downloadedAt),
                          })}
                        </Text>
                      </View>
