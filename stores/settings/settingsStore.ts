  ...
  fetchSettings: async () => {
    ...
    try {
      ...
      const themePreference =
        (storedTheme[1] as NotificationSettings["themePreference"]) || "system";
      const languagePreference = (
        storedLanguage[1] === "fr" ? "fr" : "en"
      ) as NotificationSettings["languagePreference"];
    ...
    try {
      ...
      Object.entries(partial).forEach(([key, value]) => {
        if (key === "themePreference") {
          localPartial.themePreference =
            value as NotificationSettings["themePreference"];
        } else if (key === "languagePreference") {
          localPartial.languagePreference =
            value as NotificationSettings["languagePreference"];
        } else {
