...
interface CasesState {
  ...
  fetchCaseById: (caseId: string, options?: { force?: boolean }) => Promise<void>;
...
export const useCasesStore = create<CasesState>((set, get) => ({
  ...
  fetchCases: async (filters, options) => {
    ...
    if (!force) {
      try {
        ...
        if (cached && cached.userId === userId && cached.filters === filtersKey && now - cached.fetchedAt < CASES_CACHE_TTL) {
  ...
  fetchCaseById: async (caseId: string, options) => {
    ...
    if (!force) {
      try {
        const cached = await secureStorage.get<{ case: Case; fetchedAt: number }>(cacheKey);
