...
const envSelectedApi = isProduction ? PROD_API || DEV_API : DEV_API || PROD_API;
...
const API_BASE_URL =
  ...
  "https://immigration-web-app-zeta.vercel.app/api";
      ...
      if (user) {
        try {
          ...
          if (firebaseError?.code === "auth/quota-exceeded") {
            logger.error(
              "Token quota exceeded in request interceptor - skipping token",
              firebaseError,
            );
            return config;
          }
          logger.warn(
            "Failed to get Firebase token, trying auth store",
            firebaseError,
          );
  ...
  async (error) => {
    ...
    if (error.response?.status === 401) {
      ...
      if (originalRequest._stopRetry) {
        logger.warn(
          "401 received but retries are disabled - rejecting immediately",
        );
        return Promise.reject(error);
      }
      ...
      if (retryCount >= 2 && !isAuthEndpoint) {
        ...
        if (!hasStoredUser) {
          logger.warn(
            "No stored user found after multiple 401s - logging out user",
          );
          const logout = authStore.logout;
          await logout();
        } else {
          logger.info(
            "User has stored credentials - attempting refreshAuth before logout",
          );
          try {
            ...
            logger.warn(
              "refreshAuth failed after multiple 401s - logging out user",
              refreshError,
            );
            const logout = authStore.logout;
            await logout();
        ...
        logger.warn(
          "401 received after multiple retries on auth endpoint - skipping logout to prevent loop",
        );
        return Promise.reject(error);
      }
      ...
      try {
        ...
        if (user) {
          try {
            ...
            try {
              ...
              if (retryError?.response?.status === 401) {
                logger.warn(
                  "401 persisted after token refresh - stopping retries to prevent loop",
                );
            ...
            if (
              getIdTokenError?.code === "auth/quota-exceeded" ||
              getIdTokenError?.code === "auth/network-request-failed" ||
              getIdTokenError?.message?.includes("quota-exceeded")
            ) {
              logger.error(
                "Firebase quota exceeded or network error - stopping retries immediately",
                getIdTokenError,
              );
              // Mark request to prevent further retries
              originalRequest._stopRetry = true;
          ...
          logger.info(
            "401 received with no Firebase user - attempting refreshAuth",
          );
          const authStore = getAuthStore().getState();
          ...
          try {
            ...
            if (refreshedUser) {
              ...
              try {
                ...
                try {
                  ...
                  if (retryError?.response?.status === 401) {
                    ...
                    if (!isAuthEndpoint && originalRequest._retryCount >= 2) {
                      logger.warn(
                        "401 persisted after refreshAuth - logging out user",
                      );
                      const logout = authStore.logout;
                      await logout();
                    }
                ...
                if (
                  tokenError?.code === "auth/quota-exceeded" ||
                  tokenError?.message?.includes("quota-exceeded")
                ) {
                  logger.error(
                    "Token refresh quota exceeded - stopping retries",
                    tokenError,
                  );
                  originalRequest._stopRetry = true;
                  return Promise.reject(error);
              ...
              if (!isAuthEndpoint && originalRequest._retryCount >= 2) {
                logger.info(
                  "401 received with no Firebase user and refreshAuth failed - clearing auth state",
                );
                const logout = authStore.logout;
                await logout();
              }
            ...
            logger.error(
              "refreshAuth failed during 401 handling",
              refreshError,
            );
            // Check for quota-exceeded - stop retrying immediately
            if (
              refreshError?.code === "auth/quota-exceeded" ||
              refreshError?.message?.includes("quota-exceeded")
            ) {
              logger.error(
                "Refresh auth quota exceeded - stopping all retries",
                refreshError,
              );
              originalRequest._stopRetry = true;
              return Promise.reject(error);
            ...
            if (!isAuthEndpoint && originalRequest._retryCount >= 2) {
              logger.error(
                "Token refresh failed - logging out user after retries",
              );
              const logout = authStore.logout;
              await logout();
            }
        ...
        if (
          ...
          logger.error(
            "Token refresh quota exceeded - stopping all retries",
            refreshError,
          );
          originalRequest._stopRetry = true;
          return Promise.reject(error);
