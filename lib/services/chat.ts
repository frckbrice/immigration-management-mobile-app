import {
  database,
  getDatabaseInstance as getDbInstance,
  auth,
...
class ChatService {
  private async fetchMessagesForRoom(
    ...
    try {
      ...
      if (!db) {
        logger.warn("Database is not available, returning empty messages", {
          roomId,
        });
      ...
      try {
        ...
        logger.warn("Ordered query failed, using basic query with limit", {
          roomId,
          error: error.message,
        });
      ...
      if (snapshot.exists()) {
        snapshot.forEach((childSnapshot) => {
          ...
          const mappedMessage: ChatMessage = {
            ...
            timestamp:
              firebaseData.sentAt || firebaseData.timestamp || Date.now(),
  ...
  async resolveFirebaseUserId(userId: string): Promise<string | null> {
    try {
      ...
      const looksLikeFirebaseUid =
        userId.length > 0 && userId.length < 40 && !userId.includes("-");
      if (looksLikeFirebaseUid) {
        return userId;
      }

      const response = await apiClient.get<{ data?: { firebaseId?: string } }>(
      ...
      logger.warn("Unable to resolve Firebase UID from backend identifier", {
        userId,
      });
      return null;
    } catch (error: any) {
      logger.error("Failed to resolve Firebase UID", {
        userId,
        error: error?.message,
      });
      return null;
  ...
  private mapConversationFromMetadata(
    ...
    const primaryCase =
      metadata.caseReferences && metadata.caseReferences.length > 0
        ? metadata.caseReferences[0]
        : undefined;

    const caseId = primaryCase?.caseId ?? roomId;
    const caseReference =
      primaryCase?.caseReference ?? primaryCase?.caseId ?? roomId;
  ...
  private async computeUnreadCount(
    ...
    try {
      ...
      logger.error("Failed to compute unread count for conversation", {
        roomId,
        error,
      });
      return 0;
  ...
  async loadConversations(userId: string): Promise<Conversation[]> {
    try {
      ...
      if (!db) {
        logger.warn(
          "Database not available, returning empty conversations list",
          { userId },
        );
        return [];
      }
      ...
      const conversations = await Promise.all(
        chatIds.map(async (chatId) => {
          ...
          return this.mapConversationFromMetadata(
            chatId,
            metadata,
            unreadCount,
          );
  ...
  subscribeToConversationSummaries(
    ...
    if (!currentUser) {
      logger.warn("subscribeToConversationSummaries: User not authenticated", {
        userId,
      });
      callback([]);
      return () => {};
    ...
    if (!db) {
      logger.warn(
        "Database not available, returning empty conversations subscription",
        { userId },
      );
      callback([]);
      return () => {};
    ...
    const listener = onValue(
      ...
      async () => {
        try {
          ...
          logger.error("Error loading conversations in subscription callback", {
            userId,
            error,
          });
          callback([]);
  ...
  async resolveChatRoomIdFromCase(
    caseId: string,
    clientId?: string,
    agentId?: string,
  ): Promise<string | null> {
    ...
    try {
      ...
      for (const roomId of roomIds) {
        ...
        if (metadata.caseReferences && metadata.caseReferences.length > 0) {
          const match = metadata.caseReferences.some(
            (ref) => ref.caseId === caseId,
          );
          if (match) {
            return roomId;
          }
      ...
      logger.warn("Failed to locate room id from userChats", {
        clientId,
        caseId,
        error,
      });
  ...
  async sendMessage(
    ...
    try {
      const db = getDatabaseInstance();
      if (!db) {
        logger.warn("Database not available, cannot send message", {
          caseIdOrRoomId,
          senderId,
        });
        return false;
      ...
      if (clientId && agentId) {
        chatRoomId = await this.resolveChatRoomIdFromCase(
          caseIdOrRoomId,
          clientId,
          agentId,
        );
      ...
      if (existingMetadata.exists()) {
        ...
        const { agentId: metadataAgentId, clientId: metadataClientId } =
          existingMetadata.val().participants;
  ...
  subscribeToNewMessagesOptimized(
    ...
    const db = getDatabaseInstance();
    if (!db) {
      logger.warn("Database not available, returning no-op subscription", {
        chatRoomId,
      });
      return () => {};
    }
    ...
    const unsubscribe = onChildAdded(
      ...
      (snapshot) => {
        ...
        if (lastKnownTimestamp && timestamp < lastKnownTimestamp) {
          logger.debug(
            "\n\n\n [Chat Service] Skipping message - timestamp is older than last known timestamp",
  ...
  async loadInitialMessages(
    ...
    try {
      const db = getDatabaseInstance();
      if (!db) {
        logger.warn("Database not available, returning empty messages", {
          caseId,
        });
        return { roomId: null, messages: [], hasMore: false, totalCount: 0 };
      }

      logger.info("loadInitialMessages from Firebase", {
        caseId,
        clientId,
        agentId,
      });
      ...
      if (clientId && agentId) {
        ...
        if (!resolvedRoomId) {
          const resolvedFromCase = await this.resolveChatRoomIdFromCase(
            caseId,
            clientId,
            agentId,
          );
          logger.debug("\n\n\n [Chat Service] resolvedFromCase", {
            resolvedFromCase,
          });
          if (resolvedFromCase) {
            const derivedMetadataRef = ref(
              db,
              `chats/${resolvedFromCase}/metadata`,
            );
            const derivedMetadataSnap = await get(derivedMetadataRef);
            logger.debug("\n\n\n [Chat Service] derivedMetadataSnap", {
              derivedMetadataSnap,
            });
            if (derivedMetadataSnap.exists()) {
              resolvedRoomId = resolvedFromCase;
            } else {
              const derivedMessagesRef = ref(
                db,
                `chats/${resolvedFromCase}/messages`,
              );
              const derivedMessagesSnap = await get(derivedMessagesRef);
              logger.debug("\n\n\n [Chat Service] derivedMessagesSnap", {
                derivedMessagesSnap,
              });
      ...
      if (!resolvedRoomId) {
        ...
        if (legacyMetadataSnap.exists()) {
          logger.debug("\n\n\n [Chat Service] legacyMetadataSnap exists", {
            legacyMetadataSnap,
          });
          ...
          logger.debug("\n\n\n [Chat Service] legacyMessagesSnap", {
            legacyMessagesSnap,
          });
          if (legacyMessagesSnap.exists()) {
            logger.debug("\n\n\n [Chat Service] legacyMessagesSnap exists", {
              legacyMessagesSnap,
            });
            resolvedRoomId = caseId;
          }
      ...
      if (!resolvedRoomId) {
        logger.info(
          "\n\n\n [Chat Service] No existing chat room found for case",
          { caseId },
        );
        return {
          roomId: null,
      ...
      logger.error(
        "\n\n\n [Chat Service] Error loading initial messages",
        error,
      );
      return { roomId: null, messages: [], hasMore: false, totalCount: 0 };
    }
  ...
  async loadOlderMessages(
    ...
    try {
      ...
      try {
        ...
        logger.warn(
          "\n\n\n [Chat Service] Ordered query failed, falling back to full load",
          { caseId: roomId, resolvedRoomId: roomId, error: queryError.message },
        );
  ...
  async markMessagesAsRead(roomId: string, userId: string): Promise<void> {
    try {
      const db = getDatabaseInstance();
      if (!db) {
        logger.warn("Database not available, cannot mark messages as read", {
          roomId,
          userId,
        });
        return;
      ...
      if (!snapshot.exists()) {
        logger.info("\n\n\n [Chat Service] No messages found to mark as read", {
          roomId,
        });
        return;
      }
      ...
      logger.error(
        `\n\n\n [Chat Service] Failed to mark messages as read (roomId=${roomId}, userId=${userId})`,
        error,
      );
    }
  }
  ...
  async markChatRoomAsRead(roomId: string, userId: string): Promise<boolean> {
    try {
      logger.info("\n\n\n [Chat Service] Marking chat room as read", {
        roomId,
        userId,
      });
  ...
  async initializeConversation(
    ...
    try {
      const db = getDatabaseInstance();
      if (!db) {
        logger.warn("Database not available, cannot initialize conversation", {
          caseId,
          clientId,
          agentId,
        });
      ...
      if (snapshot.exists()) {
        ...
        const caseExists = caseRefs.some(
          (ref: CaseReference) => ref.caseId === caseId,
        );
