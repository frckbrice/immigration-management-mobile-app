...
async function fetchLegalContent(endpoint: string, language: string = 'en'): Promise<string> {
    try {
        ...
        throw new Error(error?.response?.data?.error || 'Unable to load legal content');
    }
}
