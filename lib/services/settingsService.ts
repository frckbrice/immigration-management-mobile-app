...
export const settingsService = {
    async getSettings(): Promise<NotificationSettings> {
        try {
            const response = await apiClient.get<ApiResponse<SettingsResponse>>('/users/settings');
            const settings = response.data.data?.settings;
            if (!settings) {
                throw new Error(response.data.error || 'Failed to load notification settings');
            }
    ...
    async updateSettings(partial: Partial<NotificationSettings>): Promise<NotificationSettings> {
        try {
            ...
            if (payloadEntries.length === 0) {
                logger.info('Notification settings update skipped (local-only keys)', { partial });
                // Return latest backend settings to keep signature consistent
                return this.getSettings();
            }

            const payload = Object.fromEntries(payloadEntries);
            const response = await apiClient.patch<ApiResponse<SettingsResponse>>('/users/settings', payload);
            const settings = response.data.data?.settings;
            if (!settings) {
