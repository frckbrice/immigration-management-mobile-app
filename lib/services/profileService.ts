...
export const profileService = {
  ...
  async getProfile(): Promise<UserProfile> {
    try {
      ...
      if (error.response?.status === 404) {
        logger.info(
          "Profile not found (404) - user profile may not be created yet",
        );
        throw new Error(
  ...
  async changePassword(
    ...
    try {
      ...
      logger.error(
        "Error reauthenticating before changing password",
        reauthError,
      );
      ...
      switch (errorCode) {
        ...
        case "auth/network-request-failed":
          throw new Error(
            "Network error while verifying your password. Check your connection and try again.",
          );
        default:
          throw new Error(
            reauthError?.message ||
    ...
    try {
      ...
      if (error?.response?.status === 403) {
        logger.warn(
          "Falling back to Firebase updatePassword after 403 from /users/password",
        );
        try {
          ...
          logger.error(
            "Firebase updatePassword fallback failed",
            firebaseUpdateError,
          );
          const fallbackMessage =
            firebaseUpdateError?.message ||
            "Failed to change password via fallback method.";
  ...
  async exportData(): Promise<ExportDataResponse> {
    try {
      const response =
        await apiClient.get<ApiResponse<any>>("/users/data-export");
